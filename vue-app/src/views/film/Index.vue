<template>
  <div class="film">
    ...
    <div class="film-header" style="background-image: radial-gradient(farthest-side at 20% 30%, #C4A4C4 , #3A2763 );"></div>
    ...
    <div class="film-des">
      ...
      <div class="p2">Movies type : {{ film.type }}</div>
      <div class="p2">{{ film.region }} / {{ film.duration }} min</div>
      <div class="p2">Release time : {{ $moment(film.releaseTime).format('YYYY-MM-DD') }}</div>
      <div style="padding-top: 30px">
        <router-link style="margin-right: 20px" :to="'/film/ticket?fid=' + filmId">
          <el-button type="danger" style="width: 130px;letter-spacing: 2px">
            <i style="padding-right: 5px;font-size: 15px" class="el-icon-s-finance"></i>Tickets
          </el-button>
        </router-link>
        <el-button @click="openComment" type="danger" style="width: 130px;letter-spacing: 2px">
          <i style="padding-right: 5px;font-size: 15px;" class="el-icon-star-on"></i>rate
        </el-button>
      </div>
    </div>
      ...
      <el-form ref="form" :model="form" :rules="rules">
        ...
        <el-form-item prop="comment">
          <el-input
            ...
            type="textarea"
            :rows="8"
            placeholder="Please fill in the evaluation content"
  ...
  export default {
    ...
    data() {
      return {
        rules: {
          comment: [
            { required: true, message: 'Please fill out the evaluation first!', trigger: 'blur' }
          ],
        },
        value2: null,
    ...
    methods: {
      ...
      openComment() {
        if (!localStorage.getItem("uid")) {
          this.$confirm('The system has not detected your login information, should I log in?', 'tip', {
            ...
            this.$message({
              type: 'info',
              message: 'Cancel rated for Movies'
            });
          });
        } else {
