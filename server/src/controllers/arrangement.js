const dbHelper = require('../dbhelper/arrangement')
const cartDbHelper = require('../dbhelper/cart')
const orderDbHelper = require('../dbhelper/order')
...
exports.getSeats = async ctx => {
  let aid = ctx.query.id
  let result
  const cData = cartDbHelper.findAll({aid: aid})
  const oDate = orderDbHelper.findAll({aid: aid})
  result = Promise.all([cData, oDate])
  await result.then(res => {
    if (res) {
      const d = 
      ctx.body = res.flat(Infinity).map(t => t.seats && t.seats.split('号').filter(Boolean).map(t => Number(t)) || []).flat(Infinity)
    } else {
      throw new ApiError(ApiErrorNames.UNEXIST_ID)
    }
  }).catch(err => {
